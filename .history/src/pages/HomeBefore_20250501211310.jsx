...
import TechWriter from './TechWriter'; // Corrected the import
import Admin from './Admin'; // Added import for Admin component

const Home = () => {
  const [user, setUser] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [showTechWriter, setShowTechWriter] = useState(false); // State to control TechWriter component display
  ...
  // Toggle function to show or hide the TechWriter component
  const toggleTechWriter = () => {
    setShowTechWriter(!showTechWriter);
  };
    ...
    <img className='h-16' src="/kra.png" alt="Kenya Revenue Authority" />
      ...
      {user?.role === 'admin' && (
        <div className="admin-panel">
          <Admin /> {/* Render the Admin component */}
        </div>
      )}
      {user?.role === 'techwriter' && (
        <div className="techwriter-panel">
          <button 
            onClick={toggleTechWriter} 
            className="bg-blue-500 text-white py-2 px-4 rounded-md mt-4"
          >
            {showTechWriter ? 'Hide Tech Writer Panel' : 'Show Tech Writer Panel'}
          </button>

          {showTechWriter && <TechWriter />} 
        </div>
      )}
      {user?.role === 'student' && (
        <div className="student-dashboard">Welcome Student</div>
      )}
    </>
  );
};
