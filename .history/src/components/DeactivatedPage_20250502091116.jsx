...
const AdminUserManagement = ({ token }) => {
  ...
  const fetchUsers = async () => {
    console.log("Fetching users...");  // Log fetch attempt
    ...
    try {
      const res = await fetch("http://localhost:5000/api/user", {
        headers: {
          Authorization: `Bearer ${token}`,  // Using Bearer token passed as prop
        },
      });
      ...
      setError("An error occurred while fetching users.");  // Handle error
    } finally {
  ...
  useEffect(() => {
    console.log("useEffect triggered, token:", token); // Log when useEffect is triggered
    if (token) {
      fetchUsers();
    }
  }, [token]);

  console.log("Users:", users); // Log the users state after it is set
