...
const TechProfile = () => {
  ...
  // Only tech writers should see this page
  if (user.role !== 'tech_writer') {
    return <Redirect to="/profile" />;
  }

  // Split posts into Pending and Approved
  const pendingPosts  = (user.posts || []).filter(p => p.status === 'Pending');
  const approvedPosts = (user.posts || []).filter(p => p.status === 'Approved');
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      ...
      {/* Posted Content (Approved) */}
      {approvedPosts.length > 0 && (
        <div className="mt-6 bg-gray-50 p-4 rounded shadow">
          <h3 className="font-semibold mb-4">Posted Content</h3>
          {approvedPosts.map(post => (
            <div key={post.id} className="mb-6 border p-4 rounded bg-white">
              ...
              {post.image && (
                <img src={post.image} alt="" className="rounded w-full mb-2" />
              )}
              <div className="flex items-center gap-4 text-sm text-gray-700">
                <Like postId={post.id} initialCount={post.likes} />
                <AdminTechComment postId={post.id} initialCount={post.commentsCount} />
